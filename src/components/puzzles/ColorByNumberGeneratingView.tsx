...
interface ColorByNumberGeneratingViewProps {
  bookId: string;
  onPreviewBook: (bookId: string) => void;
  onExportPdf?: (bookId: string) => void;
}

export const ColorByNumberGeneratingView: React.FC<ColorByNumberGeneratingViewProps> = ({
  bookId,
  onPreviewBook,
  onExportPdf,
}) => {
  ...
  const [actionText, setActionText] = useState('Initializing geometric vector engine...');
    ...
    getPuzzleBook(bookId).then((b) => {
      if (b) {
        ...
        if (b.status === 'complete') {
          ...
          setActionText('Your Color by Number book is ready! 🎉');
        }
      }
    });

    try {
      sse = new EventSource(`/api/puzzles/color-by-number/progress/${bookId}`);
      sse.onmessage = (event) => {
        try {
  ...
  const handleExportPdf = async () => {
    setIsExporting(true);
    try {
      const response = await fetch('/api/puzzles/color-by-number/export-pdf', {
        method: 'POST',
      ...
      a.href = url;
      a.download = `${(book?.settings.title || 'color_by_number').toLowerCase().replace(/[^a-z0-9]/g, '_')}_interior.pdf`;
  ...
  const remainingScenes = Math.max(0, totalCount - completedCount);
  const estimatedSecondsLeft = remainingScenes * 2;
      ...
      <div className="bg-white rounded-3xl p-8 sm:p-12 border border-slate-200 shadow-xs text-center space-y-8">
        ...
        <div className="relative w-20 h-20 mx-auto">
          <div className={`w-20 h-20 rounded-3xl flex items-center justify-center transition-all ${
            ...
            {isComplete ? (
              ...
              <Palette size={36} className="text-teal-600 animate-spin" />
            )}
          </div>
        </div>
        ...
        <div className="space-y-2">
          <h1 className="text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-tight">
            {isComplete ? 'Color by Number Book Complete!' : 'Generating Your Color by Number Book'}
          </h1>
          <p className="text-slate-500 text-sm max-w-md mx-auto">
            {book?.settings.title || 'Segmenting geometric scenes & mapping numbered color keys...'}
          </p>
        </div>
        ...
        <div className="max-w-lg mx-auto space-y-3">
          <div className="flex justify-between items-center text-xs font-bold">
            <span className="text-slate-600 flex items-center gap-1.5">
              <span className={`w-2 h-2 rounded-full ${isComplete ? 'bg-emerald-500' : 'bg-teal-500 animate-ping'}`} />
        ...
        {isComplete && (
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-4 border-t border-slate-100">
            ...
            >
              <Eye size={16} />
              <span>Preview &amp; Edit Scenes</span>
            </button>

            <button
              onClick={handleExportPdf}
              disabled={isExporting}
              className="w-full sm:w-auto px-8 py-3.5 rounded-2xl bg-gradient-to-r from-teal-600 to-emerald-600 hover:from-teal-700 hover:to-emerald-700 text-white font-extrabold text-xs shadow-lg hover:shadow-xl transition-all active:scale-95 cursor-pointer disabled:opacity-50 flex items-center justify-center gap-2"
            >
              <Download size={16} />
