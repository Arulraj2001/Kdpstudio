...
interface ColoringGeneratingViewProps {
  bookId: string;
  onPreviewBook: (bookId: string) => void;
  onExportPdf?: (bookId: string) => void;
}

export const ColoringGeneratingView: React.FC<ColoringGeneratingViewProps> = ({
  bookId,
  onPreviewBook,
  onExportPdf,
}) => {
  ...
  const [actionText, setActionText] = useState('Initializing AI illustration pipeline...');
  ...
  useEffect(() => {
    ...
    getPuzzleBook(bookId).then((b) => {
      if (b) {
        setBook(b);
        const count = b.settings?.pageCount || 20;
        setTotalCount(count);
        if (b.status === 'complete') {
          setIsComplete(true);
          setProgress(1.0);
          setCompletedCount(b.pages?.length || count);
          setActionText('Your Coloring Book is ready! 🎉');
        }
      }
    });

    try {
      sse = new EventSource(`/api/puzzles/coloring/progress/${bookId}`);
      sse.onmessage = (event) => {
        try {
  ...
  const handleExportPdf = async () => {
    setIsExporting(true);
    try {
      const response = await fetch('/api/puzzles/coloring/export-pdf', {
        method: 'POST',
      ...
      a.href = url;
      a.download = `${(book?.settings.title || 'coloring_book').toLowerCase().replace(/[^a-z0-9]/g, '_')}_interior.pdf`;
  ...
  const remainingIllustrations = Math.max(0, totalCount - completedCount);
  const estimatedSecondsLeft = remainingIllustrations * 3;
      ...
      <div className="bg-white rounded-3xl p-8 sm:p-12 border border-slate-200 shadow-xs text-center space-y-8">
        ...
        <div className="relative w-20 h-20 mx-auto">
          <div className={`w-20 h-20 rounded-3xl flex items-center justify-center transition-all ${
            ...
            {isComplete ? (
              ...
              <Palette size={36} className="text-pink-600 animate-spin" />
            )}
          </div>
        </div>
        ...
        <div className="space-y-2">
          <h1 className="text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-tight">
            {isComplete ? 'Coloring Book Complete!' : 'Generating Your Coloring Book'}
          </h1>
          <p className="text-slate-500 text-sm max-w-md mx-auto">
            {book?.settings.title || 'Synthesizing line art illustrations & print layouts...'}
          </p>
        </div>
        ...
        <div className="max-w-lg mx-auto space-y-3">
          <div className="flex justify-between items-center text-xs font-bold">
            <span className="text-slate-600 flex items-center gap-1.5">
              <span className={`w-2 h-2 rounded-full ${isComplete ? 'bg-emerald-500' : 'bg-pink-500 animate-ping'}`} />
              <span>{actionText}</span>
            </span>
            <span className="text-pink-700 font-extrabold">
              {completedCount}/{totalCount} pages
            </span>
          </div>
        ...
        {isComplete && (
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-4 border-t border-slate-100">
            ...
            >
              <Eye size={16} />
              <span>Preview &amp; Edit Artwork</span>
            </button>

            <button
              onClick={handleExportPdf}
              disabled={isExporting}
              className="w-full sm:w-auto px-8 py-3.5 rounded-2xl bg-gradient-to-r from-pink-600 to-purple-600 hover:from-pink-700 hover:to-purple-700 text-white font-extrabold text-xs shadow-lg hover:shadow-xl transition-all active:scale-95 cursor-pointer disabled:opacity-50 flex items-center justify-center gap-2"
            >
              <Download size={16} />
                ...
                >
                  ...
                  <div className="mt-2 flex justify-center">
                    {isPageDone ? (
                      ...
                      <RefreshCw size={14} className="text-pink-600 animate-spin" />
                    ) : (
