...
export const CoverBuilderView: React.FC = () => {
  ...
  return (
    <div className="flex flex-col h-full bg-slate-100/70 text-slate-900 overflow-hidden select-none">
      ...
      <footer
        id="cover-bottom-bar"
        className="h-14 bg-white border-t border-slate-200 px-2.5 sm:px-6 flex items-center justify-between z-30 select-none shrink-0 shadow-2xs overflow-x-auto"
      >
        ...
        <div className="flex items-center gap-1 sm:gap-2 shrink-0">
          <button
            ...
            onClick={handleUndo}
            title="Undo (Ctrl+Z)"
            className="p-1.5 sm:p-2 rounded-xl hover:bg-slate-100 text-slate-600 hover:text-slate-900 transition-colors cursor-pointer"
          >
          ...
          <div className="w-[1px] h-5 bg-slate-200 mx-0.5 sm:mx-1" />

          {/* Toggle Guides */}
          <button
            type="button"
            onClick={() => setShowGuides(!showGuides)}
            className={`px-2 sm:px-3 py-1.5 rounded-xl text-xs font-semibold flex items-center gap-1.5 transition-all cursor-pointer ${
              showGuides
          ...
          >
            ...
            <span className="hidden md:inline">{showGuides ? 'KDP Guides On' : 'Guides Hidden'}</span>
          </button>
        </div>

        {/* Center: Zoom Controls */}
        <div className="flex items-center gap-1 sm:gap-2 bg-slate-100 p-1 rounded-xl border border-slate-200 shrink-0 mx-1">
          <button
            type="button"
            onClick={() => setZoomLevel((z) => Math.max(0.2, z - 0.1))}
            className="p-1 sm:p-1.5 hover:bg-slate-200 rounded-lg text-slate-600 cursor-pointer"
          >
            <ZoomOut className="w-3.5 h-3.5" />
          </button>

          <span className="font-mono text-[11px] sm:text-xs font-bold text-slate-900 px-1 sm:px-2 min-w-10 sm:min-w-14 text-center">
          ...
          <button
            ...
            title="Fit to Screen"
            className="px-1.5 sm:px-2 py-1 hover:bg-slate-200 rounded-lg text-[11px] font-semibold text-slate-700 flex items-center gap-1 cursor-pointer"
          >
        ...
        <div className="flex items-center gap-1.5 sm:gap-3 shrink-0">
          <button
            ...
            className="lg:hidden px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-xl bg-purple-50 text-purple-700 font-bold text-xs border border-purple-200 flex items-center gap-1 cursor-pointer"
          >
          ...
          <button
            ...
            className="px-2.5 sm:px-5 py-1.5 sm:py-2 rounded-xl bg-purple-600 hover:bg-purple-700 text-white font-bold text-xs shadow-sm transition-all flex items-center gap-1.5 shrink-0 cursor-pointer"
          >
