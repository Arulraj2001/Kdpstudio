...
export const POST = withUsageCheck('epubExports', async (req, { user }) => {
  try {
    ...
    // Auto-snapshot trigger on EPUB export
    if (bookId && user?.uid) {
      try {
        ...
        if (config.autoSnapshotOnExport !== false) {
          ...
          if (book) {
            snapshotId = await createSnapshot(user.uid, bookId, book, 'pre-export-epub').catch(
              (err) => {
                console.warn('Auto-snapshot on EPUB export failed:', err);
                return null;
              }
            );
          }
